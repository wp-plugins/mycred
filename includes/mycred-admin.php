...
LEFT JOIN {$wpdb->usermeta} 
	ON ({$wpdb->users}.ID = {$wpdb->usermeta}.user_id AND {$wpdb->usermeta}.meta_key = '{$cred_id}')";
		...
		public function custom_user_column_content( $value, $column_name, $user_id ) {
			...
			$ubalance = $this->core->get_users_cred( $user_id, $column_name );
			$balance = '<div id="mycred-user-' . $user_id . '-balance-' . $column_name . '">' . $this->core->before . ' <span>' . $this->core->format_number( $ubalance ) . '</span> ' . $this->core->after . '</div>';
