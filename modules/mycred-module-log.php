...
if ( ! class_exists( 'myCRED_Log_Module' ) ) {
	class myCRED_Log_Module extends myCRED_Module {
		...
		public function my_history_menu() {
			...
			$label = $mycred_types[ $this->mycred_type ];

			// Add Points History to Users menu
			$page = add_users_page(
				$label . ' ' . __( 'History', 'mycred' ),
				$label . ' ' . __( 'History', 'mycred' ),
				'read',
		...
		public function admin_page() {
			...
			if ( count( $mycred_types ) > 1 )
				$name = $mycred_types[ $this->mycred_type ];
			else
				$name = mycred_label();
...
<div class="wrap" id="myCRED-wrap">
	<h2><?php $this->page_title( sprintf( __( '%s Log', 'mycred' ), $name ) ); ?></h2>
...
<?php		$log->reset_query();
		...
		public function my_history_page() {
			...
			if ( count( $mycred_types ) > 1 )
				$name = sprintf( __( 'My %s History', 'mycred' ), $mycred_types[ $this->mycred_type ] );
			else
				$name = mycred_label();
...
<div class="wrap" id="myCRED-wrap">
	<h2><?php $this->page_title( $name ); ?></h2>
